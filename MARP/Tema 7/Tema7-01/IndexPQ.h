...
// ID es el tipo de las prioridades
// Comparator dice cuándo un valor de tipo ID es más prioritario que otro
...
public:
   IndexPQ(Comparator c = Comparator()) :
       array(1), posiciones(), antes(c) {}
   
   IndexPQ(IndexPQ<ID,T,Comparator> const&) = default;

   IndexPQ<ID,T,Comparator>& operator=(IndexPQ<ID,T,Comparator> const&) = default;
   
   ~IndexPQ() = default;
   
   // e debe ser uno de los posibles elementos
   void push(ID const& e, T const& p) {
   ...
   void update(ID const& e, T const& p) {
   ...
   T const& priority(ID const& e) const {
