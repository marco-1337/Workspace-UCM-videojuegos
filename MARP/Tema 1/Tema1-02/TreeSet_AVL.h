...
protected:
   ...
   T const & consigueKesimo (int k, Link a) const {
      ...
      if (a->tam_i == k) 
         ...
         descendant = a->dr;
         k -= a->tam_i;
      } else descendant = a->iz;

      if (descendant == nullptr) throw std::out_of_range("");

      return consigueKesimo(k, descendant);
